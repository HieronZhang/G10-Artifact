...
for policy_idx, policy in enumerate(list(policy_translation.keys())):
  for model_idx in range(len(models)):
    ...
    if model == INCEPTION:
      ...
      data_file = f"{stat_prefix}/{policy}/perf.txt"
    elif model == RESNEXT:
    ...
    for j, setting in enumerate(first_dim_arr):
      ...
      print(x_arr, y_arr)
      ax.plot(x_arr, y_arr, linestyle=line_styles[policy_idx], color=color_arr[policy_idx], marker=marker_arr[policy_idx], markerfacecolor="none", label=f"{policy_translation[policy]}", linewidth=3, markersize=9)
