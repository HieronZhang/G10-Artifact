...
def plot_timeline(ax: plt.Axes, results, filename, xlabel="Hours", ylabel="Migrate Overhead (hrs)", cumulative=False, step=False, aggregate=False, scaling=1.0, markevery=1, legend=False, yscale_log=False):
    ...
    if yscale_log:
        ...
        ax.set_yticks([0, 10737418240*0.9, 21474836480*0.9, 32212254720*0.9, 42949672960*0.9, 53687091200*0.9, 64424509440*0.9])

    ax.set_xlim(0, max(len(s) for s in results.values()))

    ax.set_yticklabels([f'{int(i/(1073741824*0.9))} GB' for i in ax.get_yticks()])
...
plot_timeline(ax, motiv1, "mem_consumption_bert", "GPU Kernel Index\n (a) GPU 2", " ", markevery=1, legend=True, yscale_log=False)
...
Figure.text(-0.2, 3.2, "Memory Consumption (Live)", rotation=90, \
